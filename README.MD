...
Set the following environment variables for the local UI/API connection:

- `NEXT_PUBLIC_API_BASE_URL` in `apps/web/.env.local` if the API is not running at `http://127.0.0.1:8000`
- `CONTOUR_ALLOWED_ORIGINS` if you need to allow browser requests from additional frontend origins
